...
async function fetchDepositRemarkByUsername(username) {
  ...
  try {
    ...
    page.on("response", async (resp) => {
      try {
        if (resp.url().includes("deposit") && resp.status() === 200) {
          const json = await resp.json();
          if (Array.isArray(json) || json?.data || json?.list || json?.items || json?.deposits) {
            searchData = json;
            logger.info("BO deposit response captured", {
    ...
    // Navigate trực tiếp với username trong URL — React SPA tự gọi API
    const now       = Date.now();
    const dateFrom  = now - 7 * 24 * 60 * 60 * 1000;
    const dateTo    = now + 24 * 60 * 60 * 1000;
    const searchUrl = `${BO_DEPOSIT_URL}?search=${encodeURIComponent(username)}&searchType=playerid&exactmatch=true&dateFrom=${dateFrom}&dateTo=${dateTo}&statusType=DEPOSIT_AUDIT&zoneType=ASIA_HO_CHI_MINH&timefilter=deposittime&sortcolumn=deposittime&sort=DESC&limit=100&offset=0&status=-1`;

    logger.info("BO browser navigating with search URL", { searchUrl });

    await page.goto(searchUrl, { waitUntil: "domcontentloaded", timeout: 60_000 });

    // Chờ API response tối đa 15s
    await page.waitForResponse(
      resp => resp.url().includes("deposit") && resp.status() === 200,
      { timeout: 15_000 }
    ).catch(() => null);
