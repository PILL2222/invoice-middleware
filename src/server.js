...
app.post("/webhook/livechat",async(req,res)=>{
  ...
  try{
    ...
    if(session.step==="collecting"){
      ...
      if(text){
        const extracted = await extractInfo(text, session);
        ...
        // Khách hỏi/phàn nàn → trả lời nhẹ nhàng + nhắc còn thiếu gì
        if(!updated && !imageUrl && (extracted.intent === "complain" || extracted.intent === "ask_status" || extracted.intent === "other")){
