...
const { fetchPendingRemark } = require("./st666api");
...
app.post("/api/urgent-invoice", upload.single("image"), async (req, res) => {
  try {
    ...
    // ── Tra Deposit Remark từ ST666 BO theo username ────────────────────────────
    let orderCode = "-";
    try {
      const pendingRemark = await fetchPendingRemark(username);
      if (pendingRemark) {
        orderCode = pendingRemark;
        logger.info("ST666 deposit remark fetched", { username, orderCode });
      } else {
        logger.warn("ST666 deposit remark not found", { username });
      }
    } catch (e) {
      logger.warn("ST666 fetchPendingRemark skipped", { error: e.message, username });
    }
