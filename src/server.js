...
app.post("/webhook/livechat",async(req,res)=>{
  ...
  try{
    ...
    // Bước 4: Đã trả kết quả — khách hỏi thêm → trả lời chung + link CSKH
...
async function processLookup(chatId,session){
  ...
  if(result.found){
    ...
    // Sau khi trả kết quả → chuyển sang step "done" để xử lý câu hỏi tiếp theo
    setSession(chatId,{...session,step:"done"});
  } else {
    const retryCount = session.retryCount||0;
    if(retryCount < 1){
      // Lần 1 không tìm thấy → yêu cầu kiểm tra lại
      await sendLivechatMessage(chatId,
      ...
      // Lần 2 vẫn không tìm thấy → link CSKH
      await sendLivechatMessage(chatId,
