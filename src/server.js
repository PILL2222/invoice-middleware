...
function isLikelyTransferContent(t){return /^[A-Za-z0-9;._\-]{4,30}$/.test(t.trim())&&!isLikelyPhone(t);}
function extractTransferContent(text){
  if(!text) return null;
  const lb=text.match(/(?:nội dung|nd|ck|content)[:\s]+([A-Za-z0-9;._\-]{4,50})/i);
...
// Parse LiveChat webhook payload
function parseLivechatPayload(body) {
  ...
  const authorId = body?.payload?.event?.author_id || body?.event?.author_id || body?.author_id || null;
...
app.post("/webhook/livechat", async (req,res) => {
  ...
  try {
    ...
    logger.info("WEBHOOK",{chatId,text:text.slice(0,60),authorId,eventType,hasImage:!!imageUrl});
    ...
    // Bỏ qua tin của agent (email format = agent)
    if(!isCustomerAuthor(authorId)){
      logger.info("Skipping agent message",{authorId});
      return;
    }
    ...
    if(session.step==="collecting"){
      let updated=false;
      if(imageUrl&&!session.imageUrl){session={...session,imageUrl};updated=true;}
      if(text&&!session.phone&&isLikelyPhone(text)){
        const phone=cleanPhone(text);
        if(isValidPhone(phone)){session={...session,phone};updated=true;}
        else{await sendLivechatMessage(chatId,"Dạ số điện thoại chưa đúng (10 số, bắt đầu 0), nhập lại nhé ạ 🙏");return;}
      }
  ...
  } catch(err){logger.error("Handler error",{error:err.message});}
});
...
async function processLookup(chatId,session){
  ...
  if(!imageBuf){await sendLivechatMessage(chatId,"Dạ em không tải được ảnh, anh/chị gửi lại ảnh giúp em nhé ạ 🙏");setSession(chatId,{...session,step:"collecting",imageUrl:null});return;}
  let result;
  ...
  catch(err){logger.error("Search error",{error:err.message});await sendLivechatMessage(chatId,"Dạ hệ thống gặp sự cố, em chuyển anh/chị sang nhân viên hỗ trợ ngay ạ...");await transferToAgent(chatId,`Lỗi — User:${session.username} SĐT:${session.phone}`);clearSession(chatId);return;}
  if(result.found){
    ...
    await sendLivechatMessage(chatId,"Dạ em không tìm thấy hóa đơn khớp ạ 😔\nĐang kết nối nhân viên hỗ trợ cho anh/chị...");
