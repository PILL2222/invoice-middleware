 ...
 * Khách gõ: "họ tên: Nguyễn Văn A, ck: CKFP5e0h" → hiểu hết
 * Khách gõ: "Nguyễn Văn A CKFP5e0h" → hiểu hết
 */
function extractAll(text, session) {
  const result = {};
  if (!text) return result;

  // 1. Tìm Họ Tên — có label hoặc standalone (chuỗi chữ thuần + khoảng trắng)
  if (!session.fullname) {
    const labeled = text.match(/(?:h[oọ]\s*t[eê]n|t[eê]n\s*(?:chủ\s*tk|chủ)?|fullname|name)[:\s]+([A-Za-zÀ-ỹ\s]{3,50})/i);
    if (labeled) {
      result.fullname = labeled[1].trim();
    } else {
      // Standalone: chuỗi chữ cái (có dấu hoặc không) dài 5-50 ký tự, có khoảng trắng (2+ từ)
      const nameMatch = text.match(/^([A-Za-zÀ-ỹ][A-Za-zÀ-ỹ\s]{4,49})$/m);
      if (nameMatch && nameMatch[1].trim().split(/\s+/).length >= 2) {
        result.fullname = nameMatch[1].trim();
      }
    }
  }

  // 2. Tìm CK code / Mã giao dịch
  if (!session.transferContent) {
    ...
    // Standalone token — CK code có thể bắt đầu bằng số hoặc chữ
    if (!result.transferContent) {
      ...
      for (const token of allTokens) {
        ...
        // CK code: có cả chữ lẫn số (bất kể bắt đầu bằng gì)
        if (/[A-Za-z]/.test(t) && /[0-9]/.test(t)) {
          result.transferContent = t; break;
        }
...
// Hoàn chỉnh nếu có đủ để tìm
function isComplete(s){
  const hasIdentifier = s.transferContent;
  const hasImage = s.imageUrl;
  const hasFullname = s.fullname;
  return hasIdentifier && (hasImage || hasFullname);
}
