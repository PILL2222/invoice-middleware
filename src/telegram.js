...
function parseCaption(text) {
   ...
   *   Dòng 1: Username
   *   Dòng 2: Họ Tên
   *   Dòng 3: Nội dung CK hoặc Mã giao dịch
   *   Dòng 4: Mã đơn (chỉ lưu nội bộ, không gửi khách)
   *   Dòng 5: Trạng thái
   */
  if (!text) return {};
  const lines = text.split(/\n+/).map(l => l.trim()).filter(Boolean);

  // Dòng 1: username, Dòng 2: họ tên — cố định theo vị trí
  const username = lines[0] || null;
  const fullname = lines[1] || null;

  let ckCode=null, orderCode=null, status=null;
  ...
  for (let i=2; i<lines.length; i++) {
    ...
    // Số thuần >= 6 chữ số: CK (dòng 3) hoặc Mã đơn (dòng 4)
    if (/^\d{6,30}$/.test(line)) {
      if (!ckCode) { ckCode=line; continue; }
      else { orderCode=line; continue; }
    }

    // CK code: chữ + số (bắt đầu bằng chữ HOẶC số)
    if (!ckCode && /^[A-Za-z0-9]{4,30}$/.test(line) && /[A-Za-z]/.test(line) && /[0-9]/.test(line)) {
      ckCode=normCK(line); continue;
    }

    // Dòng còn lại → mã đơn
    if (!orderCode) orderCode=line;
  }

  // Fallback: tìm CK trong "ACB;48525327;CKFP5e0h"
  if (!ckCode) {
    const m = text.match(/;([A-Za-z]{2,}[0-9][A-Za-z0-9]{1,})\b/);
    if (m) ckCode=normCK(m[1]);
  }

  return { username, fullname, ckCode, orderCode, status, note: null };
}
...
module.exports={searchInvoiceByAll,telegramService,addRuntimeStatus};
