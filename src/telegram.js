...
async function handleT3Reply(msg) {
  ...
  // Format: Ngày giờ / ID / Ghi chú / Trạng thái
  const root       = imageCache.get(Number(pending.rootId));
  ...
  // Index relay message vào cache để admin có thể reply tiếp → chain tiếp tục được
  // Bot không tự nhận webhook của chính mình nên phải tự index ở đây
  if (relayMsg?.message_id) {
    const relayMsgId = Number(relayMsg.message_id);
    textCache.set(relayMsgId, {
      message_id:   relayMsgId,
      parent_id:    Number(pending.rootId),
      message_date: Date.now(),
      status:       pending.status,
      note:         note || null,
      cached_at:    Date.now(),
    });
    addReply(Number(pending.rootId), relayMsgId);
    saveCache();
    logger.info("Relay message indexed", { relayMsgId, rootId: pending.rootId, status: pending.status });
  }
