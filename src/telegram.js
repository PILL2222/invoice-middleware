...
function parseCaption(text) {
   ...
   *   CKFP5e0h
   *   ĐH-0042
   *   Khách chuyển đúng nội dung
   *   Đang xử lý
   *
   * Ví dụ (5 dòng - không có ghi chú):
   *   myhanh1233
   *   Nguyễn Thị Mỹ Hạnh
   *   CKFP5e0h
   *   ĐH-0042
  ...
  // Dòng 3: CK code — cố định theo vị trí
  const ckCode = lines[2] ? normCK(lines[2]) : null;

  // Dòng 4: mã nội bộ — cố định theo vị trí (không gửi khách)
  const orderCode = lines[3] || null;

  // Từ dòng 5 trở đi: tìm trạng thái (dòng cuối) + ghi chú
  let status = null;
  let note = null;
  ...
  for (let i = 4; i < lines.length; i++) {
    ...
    if (s) { status = s; }
    else if (!note) { note = lines[i]; }
  }

  // Fallback: nếu chưa có status, dò lại từ dòng 3 trở đi
  if (!status) {
