...
function parseCaption(text) {
  ...
  /**
   * Parse THEO VỊ TRÍ DÒNG — không đoán kiểu dữ liệu:
   *   Dòng 1: username       (luôn là dòng đầu tiên)
   *   Dòng 2: SĐT            (nhận dạng bằng format số điện thoại)
   *   Dòng 3: CK code        (dòng còn lại sau khi trừ SĐT và status)
   *   Dòng 4: Trạng thái     (khớp STATUS_KW)
   *   Dòng 5+: Ghi chú
   */

  // Dòng 1 luôn là username — bất kể nội dung
  ...
  for (let i=1; i<lines.length; i++) {
    ...
    // Đã có status → phần còn lại là ghi chú
    if (statusFound) { noteLines.push(line); continue; }
    ...
    if (s && !status) {
      status=s; statusFound=true;
      const rem=line.replace(new RegExp(s.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),"gi"),"").replace(/[✅❌⏳🔄↩️⚠️📋]/g,"").trim();
      if (rem) noteLines.push(rem);
      continue;
    }

    // Dòng còn lại (sau username và SĐT) → CK code
    if (!ckCode && line.length>=4 && line.length<=30) {
      ckCode=normCK(line);
      continue;
    }
  }
...
function parseReplyText(text) {
  ...
  if (status && lines.length>1) {
    note=lines.filter(l=>!detectStatus(l)).join(" | ")||null;
  }
  return {status,note};
}
