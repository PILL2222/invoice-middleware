...
function parseReplyText(text) {
  /**
   * FORMAT REPLY (tối thiểu 3 dòng):
   *   Dòng 1: Ghi chú (tùy chọn)
   *   Dòng 2: Ghi chú (tùy chọn)
   *   Dòng 3+: Trạng thái mới
   *
   * Trường hợp đặc biệt:
   *   1 dòng → trạng thái luôn (không có ghi chú)
   *   2 dòng → dòng 1 ghi chú, dòng 2 trạng thái
   *   3+ dòng → dòng 1+2 ghi chú, dòng 3+ trạng thái ← FORMAT CHUẨN
   */
  if (!text) return { status: null, note: null };
  const lines = text.split(/\n+/).map(l => l.trim()).filter(Boolean);

  if (lines.length === 1) {
    // 1 dòng → chỉ có trạng thái
  ...
  // 3+ dòng (FORMAT CHUẨN) → dòng 1+2 ghi chú, dòng 3+ trạng thái
  const note   = [lines[0], lines[1]].join(" | ");
  const statusText = lines.slice(2).join(" ");
