...
function parseCaption(text) {
  ...
  for (const line of lines) {
    ...
    if (s && !status) {
      ...
      const rem=line.replace(new RegExp(s,"gi"),"").replace(/[✅❌⏳🔄↩️⚠️📋]/g,"").trim();
      if (rem) noteLines.push(rem);
      continue;
    }
    if (!ckCode && isCKCode(line)) { ckCode=normCK(line); continue; }
    if (!username && line.length>=2 && line.length<=50) { username=line; }
  }

  // Tìm CK trong chuỗi dạng "ACB;48525327;CKFP5e0h"
  if (!ckCode) {
