...
// ── Check xem deposit có đang pending không ───────────────────────────────────
// status = 1 hoặc statusType chứa "pending" (tuỳ BO system)
function isPending(deposit) {
  if (deposit.status === 1) return true;
  if (deposit.status === "1") return true;
...
async function fetchPendingRemark(username) {
  ...
  try {
    ...
    // Lọc đơn pending
    const pending = deposits.filter(isPending);

    if (!pending.length) {
      logger.info("ST666 no pending deposit", { username });
      return null;
    }
